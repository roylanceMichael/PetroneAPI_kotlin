...
internal class CharReadChannelImpl(val source: ReadChannel, val charset: Charset,
    val buffer: ByteBuffer) : CharReadChannel {
...
fun ReadChannel.asCharChannel(charset: Charset = Charsets.UTF_8,
    buffer: ByteBuffer = ByteBuffer.allocate(8192)): CharReadChannel = CharReadChannelImpl(this,
    charset, buffer)
 ...
 */
suspend fun <A : Appendable> CharReadChannel.readLineTo(out: A,
    buffer: CharBuffer): Pair<A, CharBuffer> {
  while (true) {
    val eof = fill(buffer)
