...
class InterestSuspensionsMap {
  ...
  fun addSuspension(interest: SelectInterest, continuation: CancellableContinuation<Unit>) {
    ...
    if (continuation is CancellableContinuation<*>) {
      continuation.invokeOnCompletion {
        if (continuation.isCancelled) dropHandler(interest, continuation)
      }
    }
  }

  inline fun invokeIfPresent(interest: SelectInterest,
      block: CancellableContinuation<Unit>.() -> Unit): Boolean {
  ...
  fun removeSuspension(interest: SelectInterest): CancellableContinuation<Unit>? = updater(
      interest).getAndSet(this, null)
  ...
  companion object {
    ...
    private val updaters = SelectInterest.values().map { interest ->
      AtomicReferenceFieldUpdater.newUpdater(InterestSuspensionsMap::class.java,
          ...
          "${interest.name.toLowerCase()}HandlerReference") as AtomicReferenceFieldUpdater<InterestSuspensionsMap, CancellableContinuation<Unit>?>
    }.toTypedArray()

    private fun updater(
        interest: SelectInterest): AtomicReferenceFieldUpdater<InterestSuspensionsMap, CancellableContinuation<Unit>?> = updaters[interest.ordinal]
  }
}